...
plot_filename = os.path.join(os.getcwd(), 'figures', f'Iris_cost_{hidden_neurons}_hidden_neuron.png')
plt.savefig(plot_filename)
min_cost = np.min(errors)
argmin_cost = np.argmin(errors)
print('Minimum cost: {:.5f}, on iteration #{}'.format(min_cost **2, argmin_cost+1))

# Plot the train & test cost for all iterations
fig, ax1 = plt.subplots()
plot_cost_train_test(cost_train, cost_test, ax1)
plot_filename = os.path.join(os.getcwd(), 'figures', f'Iris_train_test_cost_{hidden_neurons}_hidden_neuron.png')
plt.savefig(plot_filename)
min_cost = np.min(cost_train)
test_cost = np.min(cost_test)
print('Minimum cost: {:.5f}, test cost: {:.5f}'.format(min_cost, test_cost))
