...
def test_xor(patterns_input, patterns_output, nn):
    ...
    for p in range(n_samples):
        ...
        print('Sample #{:02} | Target value: {:.2f} | Predicted value: {:.5f}'.format(p+1, target[0], prediction))
...
def test_iris(inputs, targets, nn):
    ...
    for p in range(0, 5):
        ...
        print('Sample #{:02} | Target value: {:.2f} | Predicted value: {:.5f}'.format(p+1, np.argmax(target), np.argmax(prediction)), prediction)
