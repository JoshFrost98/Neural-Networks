...
class NeuralNetwork():
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        hidden_layer_outputs = self.y_hidden.copy()
        for k in range(self.n_out):
            #########################################
            # Write your code here
            # compute output_deltas : delta_k = (y_k - t_k) * g'(x_k)
            delta_k = (outputs[k] - targets[k]) * sigmoid_derivative(outputs[k])
            output_deltas[k] = delta_k
            ########################################/
        ...
        for j in range(len(hidden_deltas)):
            ...
            delta_j = sum(output_deltas[:] * self.w_out[j,:]) * sigmoid_derivative(hidden_layer_outputs[j])
            hidden_deltas[j] = delta_j
        ...
        for j in range(len(self.y_hidden)):
            for k in range(len(output_deltas)):
                ...
                self.w_out[j,k] -= output_deltas[k] * self.y_hidden[j] * learning_rate
                ########################################/
        ...
        for i in range(len(inputs)):
            for j in range(len(hidden_deltas)):
                ...
                self.w_hidden[i,j] -= hidden_deltas[j] * inputs[i] * learning_rate
                ########################################/
